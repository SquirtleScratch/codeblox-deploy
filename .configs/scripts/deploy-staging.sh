  ...
  aws elasticbeanstalk create-application-version --application-name codeblox-staging --version-label $GITHUB_SHA --source-bundle S3Bucket=$EB_BUCKET,S3Key=codeblox.zip
  ...
  aws elasticbeanstalk update-environment --environment-name codeblox-staging-env --version-label $GITHUB_SHA